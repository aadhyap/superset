...
- `SupersetMetastoreCache` no longer falls back to `PickleKeyValueCodec` when a cache config sets `CACHE_TYPE: "SupersetMetastoreCache"` without a `CODEC`; the default is now `JsonKeyValueCodec()`, so unsafe pickle deserialization requires an explicit opt-in. The shipped `FILTER_STATE_CACHE_CONFIG`, `EXPLORE_FORM_DATA_CACHE_CONFIG` and `EXTENSIONS_EPHEMERAL_STORAGE` defaults already configure the JSON codec and are unaffected. Deployments that relied on the implicit pickle codec (i.e. that cache values JSON cannot represent, such as tuples or arbitrary Python objects) must set `"CODEC": PickleKeyValueCodec()` in the affected cache config to keep that behaviour. Entries written with a different codec than the configured one are treated as cache misses instead of raising, so existing pickle-encoded entries are transparently regenerated after the switch.
